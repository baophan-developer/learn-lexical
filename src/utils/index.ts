...
export const toHex = (value: string) => {
	if (!value.startsWith("#")) {
		...
		if (!ctx) {
			throw new Error("2d context do not supported or canvas already initialized.");
		}
...
export const hex2rgb = (hex: string): RGB => {
	...
	return {
		r: rgbArr[0] as number,
		g: rgbArr[1] as number,
		b: rgbArr[2] as number,
	};
};
...
export const transformColor = <M extends keyof Color, C extends Color[M]>(
	format: M,
	color: C,
): Color => {
